...
export const CartPage = () => {
  ...
  useEffect(() => {
    const totalPrice = CartList.reduce(
      (prev, currentProduct) =>
        prev + currentProduct.price * currentProduct.quantity,
      0
    );
  ...
  return (
    <>
      {contextHolder}
      {CartList.length ? (
        <Title style={{ textAlign: "center" }}>Cart</Title>
      ) : (
        <Title style={{ textAlign: "center" }}>現在カート内は空です</Title>
      )}
      {isModalOpen && (
      ...
      >
        <div>
          {CartList.map((product) => {
            return (
              <div
                key={product.id}
                style={{ border: "solid", marginBottom: "8px" }}
              >
                <Row>
                  <Col span={8}>
                  ...
                  >
                    ...
                    <Text strong>
                      ...
                      <Select
                        defaultValue={1}
                        style={{ width: "56px" }}
                        options={PURCHASE_QUANTITY_OPTIONS}
                        onChange={(value) =>
                          changeProductQuantity(value, product.id)
                        }
                      />
                    </Text>
                    <Button
        ...
        {CartList.length > 0 && (
          ...
          >
            ...
            <Button type="primary" onClick={completePurchase}>
              購入
            </Button>
          </div>
        )}
